...
uses tiny, hand-built graphs in which *exactly one* causal structure is active:
...
Output (errors are MSE normalized by the target's own variance; lower is
better, 1.0 = predicting the mean):
...
Usage:
    python test_constraints_continuous.py
    python test_constraints_continuous.py --n-seeds 3 --networks mlp,transformer
    # fast smoke test:
    python test_constraints_continuous.py --n-seeds 1 --n-epochs 3 --n-runs 2 --networks mlp
"""
import argparse
import logging
...
def build_structure_dataset(edges, n_samples, seed,
                            w_ranges=((-2.0, -0.5), (0.5, 2.0)), noise_scale=1.0):
    """Sample a dataset from a fixed linear-Gaussian DAG (given by `edges`) over NODES.

    Reuses notears_util's SEM parameter/sampling machinery (the same building
    blocks continuous_er_graph.py wraps) so the data-generating process
    matches the random-graph experiments exactly; only the adjacency is fixed
    instead of random.
    Returns (DataFrame over NODES, binary adjacency matrix B with B[i,j]=1 => i->j).
    """
    np.random.seed(seed)
    d = len(NODES)
    ...
    X = simulate_linear_sem(W, n_samples, 'gauss', noise_scale)
...
def main():
    ...
    parser.add_argument("--out", default="constraint_comparison_continuous",
    ...
    parser.add_argument("--noise-scale", type=float, default=1.0,
                        help="additive Gaussian noise scale per node (default 1.0)")
    ...
    for seed in seeds:
        for struct, edges in STRUCTURES.items():
            data, B = build_structure_dataset(
                edges, args.n_samples, seed, noise_scale=args.noise_scale)

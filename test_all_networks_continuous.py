...
def fit_violation(prep_data, w_est, row_and_col_names, target, features,
    ...
    reuse ContinuousRecommenderPredictor.constraint_violation. Returns the
    total violation (NaN on failure). Honours the cfg restriction flags
    ...
    try:
        ...
        y = prep_data[target]
        model = ContinuousRecommenderPredictor(
